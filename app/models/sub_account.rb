class SubAccount < ActiveRecord::Base
  ...
  def sub_account_period_balance(sub_account, month, year)  
      ...
      (sub_account.transaction_items.where(:debit => true, :transaction_id => transactions).sum("amount") * sub_account.account.bs_category.debit_impact) \
      + (sub_account.transaction_items.where(:debit => false, :transaction_id => transactions).sum("amount") * sub_account.account.bs_category.credit_impact)
  end
